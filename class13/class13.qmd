...
Today we wil analyze some RNASeq data from Himes et al. on the effects of dexamrthasone (dex), a synthetic glucocorticoid steroid on air smooth muscle cells (ASM).
...
> Q. How many genes are "down" regulated upon drug treatment at a threshold of -2 log2-fc??
...
The main function in the package is called `DESeq()` and we can run it on our `dds` object
...
Make a common overall results figure from this analysis. This is designed to keep our iner biologist and inner stats nerd happy - it plots fold-change vs P-value
...
To fo this "annotation" I will use the "AnnotationDbi" package. I can install this with `BiocManager::install()`
...
I will use `mapIds()` function to "map" my identifiers to those from different databases. I will go between "ENSEMBL" and "SYMBOL" (and then after "GENENAME").
...
Let's use the *gage* package to look for KEGG pathways in our results (genes of interest). I will also use the *pathview* package
...
Let's use the pathview package to look at one of these highlighted KEGG pathways with our genes highighted. "hsa05310 Asthma"
