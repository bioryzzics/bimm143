...
Pertussis, aka whooping cough, is a highly infectious lunch disease
caused by the bacteria *B. Pertussis*.

The CDC tracks pertussis cases numbers per year. Lets have a closer look
at this data:
...
available lots of a data about the immune response to Pertussis booster
vaccination.

Critically, it tracks wP and aP individuals over time to seee how their
...
> Q4. Breakdown by biological_sex and race, e.g. how many black female
> subjects etc.
...
Now we can join (merge) these two tables `subject` amd `specimen` to
...
Let’s focus on the IgG - one of the main antibody types responsive to
...
Let’s try a different plot. First focus on one antigen, start with
PT(Pertussis toxin) and plot visit. or time on the x-axis and
